...
function ProcessoLista() {
    ...
    useEffect(() => {
        carregarDados();
    }, [pagination.current, pagination.pageSize, searchText, filtroSituacao, filtroRegime, filtroPrazo]);
    ...
    const carregarDados = async () => {
        ...
        try {
            ...
            const response = await getProcessos(params.page, params.size, params);
    ...
    const handleViewDetails = async (record) => {
        ...
        if (record.clienteId) {
            ...
            const cliente = data.find(c => c.id === record.clienteId);
        ...
        setTimeout(() => {
            ...
            form.setFieldsValue({
                ...
                lapsoProgressao: record.lapsoProgressao ? dayjs(record.lapsoProgressao) : null,
                situacao: record.situacao,
                regimePrisional: record.regimePrisional,
                matriculaSap: record.matriculaSap,
            });
    ...
    const handleModalOk = async () => {
        ...
        try {
            ...
            const dataToSend = {
                ...
                matriculaSap: values.matriculaSap,
                regimePrisional: values.regimePrisional,
                prazoEmAberto: values.prazoEmAberto,
                dataPrazo: values.dataPrazo ? values.dataPrazo.format('YYYY-MM-DD') : null,
                lapsoProgressao: values.lapsoProgressao ? values.lapsoProgressao.format('YYYY-MM-DD') : null,
    ...
    const columns = [
        ...
        { title: 'Nº do processo', dataIndex: 'numeroProcesso', render: (text, record) => (
            <Button type="link" style={{ padding: 0, color: record.lapsoProximo ? '#ef4444' : '#1a3a5c', fontWeight: record.lapsoProximo ? 'bold' : 'normal' }}  onClick={() => handleViewDetails(record)}> {text || '-'} </Button>
        ),},
        ...
        { title: 'Situação', dataIndex: 'situacao', width: 100, render: (text) => {
            ...
            return (
                <Tag color={getStatusColor(text)} style={{ fontSize: 11 }}> {found ? found.label : text || '-'} </Tag>
            );
        ...
        { title: 'Cliente', dataIndex: 'clienteNome', width: 150, render: (text, record) => {
            ...
            if (record.clienteId && record.clienteNome) return record.clienteNome;
                return text || '-';
            },
        ...
        { title: 'Regime', dataIndex: 'regimePrisional', width: 130, render: (text) => getRegimeLabel(text), },
                ...
                <Row gutter={16}>
                    ...
                    <Col xs={24} sm={12} md={12}>
                        ...
                        <Form.Item name="clienteId" label="Cliente">
                            ...
                            <Select size="small"showSearch={{ filterOption: (input, option) => (option?.label ?? '').toLowerCase().includes(input.toLowerCase()), }} placeholder="Selecione o cliente" options={clientesOptions} loading={clientesOptions.length === 0} onChange={(value, option) => {
                                
                                const cliente = data.find((c) => c.id === value);
                                
                                if (cliente) {
                                    setClienteSelecionado(cliente);
                                    form.setFieldsValue({ matriculaSap: cliente.matriculaSap || '', regimePrisional: cliente.regimePrisional || null, });
                                }
                ...
                <Row gutter={16}>
                    ...
                    <Col xs={24} sm={12} md={8}>
                        ...
                        <Form.Item name="matriculaSap" label="Matrícula SAP" style={{ marginBottom: 8 }}>
                            <Input size="small" disabled={true} style={{ backgroundColor: '#f5f5f5' }} />
                        </Form.Item>
                    ...
                    <Col xs={24} sm={12} md={8}>
                        ...
                        <Form.Item name="regimePrisional" label="Regime prisional" style={{ marginBottom: 8 }}>
                            <Select size="small" options={REGIME_PRISIONAL_OPTIONS} disabled={true} style={{ backgroundColor: '#f5f5f5' }} />
                        </Form.Item>
                    ...
                    <Col xs={24} sm={12} md={8}>
                        ...
                        <Form.Item name="lapsoProgressao" label="Lapso para progressão">
                            <DatePicker style={{ width: '100%' }} format="DD/MM/YYYY" size="small" />
                        </Form.Item>
    ...
    <div style={{ padding: isMobile ? 8 : 16 }}>
            ...
            <Row gutter={[12, 12]} justify="space-between" align="middle">
                ...
                <Col xs={24} md={16}>
                    ...
                    <Space wrap style={{ width: isMobile ? '100%' : 'auto' }}>
                        ...
                        <Input placeholder="Buscar por processo, cliente ou matrícula" value={searchText} onChange={handleSearch} style={{ width: isMobile ? '100%' : 250 }} prefix={<SearchOutlined />} allowClear />
                        ...
                        {!isMobile && (
                            ...
                            <>
                                ...
                                <Select placeholder="Regime" allowClear style={{ width: 130 }} value={filtroRegime} onChange={handleRegimeChange} options={REGIME_PRISIONAL_OPTIONS} />
                                <Select placeholder="Prazo em aberto?" allowClear style={{ width: 130 }} value={filtroPrazo} onChange={handlePrazoChange} options={[{ value: true, label: 'Sim' }, { value: false, label: 'Não' }]} />
                            </>
                ...
                <Col xs={24} md={8} style={{ textAlign: isMobile ? 'center' : 'right' }}>
                    <Button type="primary" onClick={handleAdd} icon={<PlusOutlined />} style={{ background: '#131a53', borderColor: '#131a53', width: isMobile ? '100%' : 'auto', transition: 'all 0.3s ease' }} className="btn-novo-processo" disabled={isReadOnly}> Novo processo </Button>
                </Col>
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            {data.map((record) => {
                                ...
                                const regimeLabel = getRegimeLabel(record.regimePrisional);
                                const isLapsoProximo = record.lapsoProximo || false;

                                return (
                                
                                <Card key={record.id} size="small" style={{ marginBottom: 8, borderRadius: 6, borderColor: isLapsoProximo ? '#ef4444' : undefined, borderWidth: isLapsoProximo ? 2 : undefined, }} styles={{ body: { padding: '8px 10px' } }}>
                                    
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                                        
                                        <Typography.Text strong style={{ color: isLapsoProximo ? '#ef4444' : '#1a3a5c', fontSize: 13 }}>
                                            {record.numeroProcesso || 'Nº não informado'}
                                            {isLapsoProximo && <Tag color="red" style={{ fontSize: 9, marginLeft: 4 }}>Urgente</Tag>}
                                        </Typography.Text>
                                    ...
                                    <Row gutter={[6, 4]}>
                                        ...
                                        <Col span={12}>
                                            ...
                                            <div>
                                                <Tag color={getStatusColor(record.situacao)} style={{ fontSize: 10, margin: 0, padding: '0px 6px', lineHeight: '18px' }}> {situacaoLabel} </Tag>
                                            </div>
                                        ...
                                        <Col span={12}>
                                            ...
                                            <div style={{ fontSize: 11 }}>{regimeLabel}</div>
                                        </Col>
                                    ...
                                    {record.honorarios && (
                                        ...
                                        <Row gutter={[6, 4]}>
                                            ...
                                            <Col span={24}>
                                                ...
                                                <div style={{ fontSize: 11, fontWeight: 500, color: '#1a3a5c' }}> R$ {record.honorarios.toLocaleString('pt-BR')} </div>
                                            </Col>
        ...
        <Drawer title={<span style={{ color: '#1a3a5c' }}>Filtros</span>} placement="bottom" onClose={() => setFiltersDrawerOpen(false)} open={filtersDrawerOpen} size="auto">
            ...
            <Space orientation="vertical" style={{ width: '100%' }} size="middle">
                ...
                <Select placeholder="Regime prisional" allowClear style={{ width: '100%' }} value={filtroRegime} onChange={handleRegimeChange} options={REGIME_PRISIONAL_OPTIONS} />
        ...
        <Modal title={!editingItem ? 'Novo processo' : (isEditMode ? 'Editar processo' : 'Visualizar processo')} open={modalVisible} onCancel={handleCancelModal} width={isMobile ? '90%' : 700} mask={{ closable: false }} footer={
            ...
            <Button key="cancel" onClick={() => {
                ...
                if (editingItem) {
                    ...
                    form.setFieldsValue({
                        ...
                        lapsoProgressao: editingItem.lapsoProgressao ? dayjs(editingItem.lapsoProgressao) : null,
                    });
